...
namespace NetworkServerTest {
	void ResetQueues() {
		Singleton<BlockingQueue<TCPArrivedNetworkPackage>>().Reset();
		Singleton<BlockingQueue<TCPResponseNetworkPackage>>().Reset();
	}
	...
	TEST(NetworkServerTest, ServerSendGivenPackage) {
		Singleton<LogMessageProcessor>().Suspend();
		ResetQueues();
		NetworkServer server;
		MessageProcessor messageProcessor;
		MessageProcessorRTSPMessage msgProcessorrtsp;

		messageProcessor.Subscribe(std::bind(&MessageProcessorRTSPMessage::ProcessNetworkPackage, &msgProcessorrtsp, _1));

		messageProcessor.Start(1);
		server.StartServer("127.0.0.10"s, { 554 }, 1);
